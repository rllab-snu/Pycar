...
class Car2():
    def __init__(self, color, x, y, screen, speed=0):
        ...
        if (screen != None):
            if self.color==RED:
                ...
                self.originalImage = pygame.image.load(
                    "images/green_car.png").convert_alpha()
            elif self.color==BLUE:
                self.originalImage = pygame.image.load(
                    "images/blue_car.png").convert_alpha()
            else:
                self.originalImage = pygame.image.load(
                    "images/red_car.png").convert_alpha()
        ...
        self.initial_state = (self.pose[0], self.pose[1], self.angle,
    ...
    def reset(self):
        self.sliding_history = np.zeros((1, 10))
        self.pose = [self.initial_state[0], self.initial_state[1]]
        self.angle = self.initial_state[2]
        self.steering_angle = self.initial_state[3]
        self.vel = [self.initial_state[4], self.initial_state[5]]
        self.speed = self.initial_state[6]
        self.timer = 0
        self.recent_action = 0
        if (self.screen != None):
            self.rect = self.image.get_rect()
